...
export function memeFranchise(a: Titre, b: Titre): boolean {
 ...
 */
export function genreDeReference(titre: Titre, adores: Set<string>): string {
...
function respecteQuotas(
  ...
  quotaDe: (genre: string) => number = () => DIVERSITE.maxParGenrePrincipal,
  genreDe: (t: Titre) => string = genrePrincipal,
): boolean {
 ...
 */
export function ordonnerPourAffichage(liste: Candidat[], idsPepites: Set<string>): Candidat[] {
...
export function assurerPlancherPertinence(
  ...
  vivier: Candidat[],
  genresAdores: string[],
  idsPepites: Set<string>,
  part: number = DIVERSITE.plancherPertinence,
): Candidat[] {
  if (genresAdores.length === 0 || liste.length === 0) return liste;
  const adores = new Set(genresAdores);
  const pertinent = (c: Candidat) => c.titre.genres.some((g) => adores.has(g));
  ...
  const genreDe = (t: Candidat['titre']) => genreDeReference(t, adores);
  const compteurRealisateur = new Map<string, number>();
  const compteurGenre = new Map<string, number>();
...
export function assurerPlancherActualite(
  ...
  fenetreAnnees: number,
  idsPepites: Set<string>,
  genresAdores: string[],
  /** Sert à ne jamais sacrifier le dernier titre de la jeunesse. */
  estNostalgique: (c: Candidat) => boolean = () => false,
  part: number = DIVERSITE.plancherActualite,
): Candidat[] {
  if (liste.length === 0) return liste;
  const recent = (c: Candidat) => anneeCourante - c.titre.annee <= fenetreAnnees;
  ...
  const adores = new Set(genresAdores);
  const pertinent = (c: Candidat) => c.titre.genres.some((g) => adores.has(g));
  const genreDe = (c: Candidat) => genreDeReference(c.titre, adores);
  const quotaDe = (genre: string) =>
    adores.has(genre) ? DIVERSITE.maxParGenreAdore : DIVERSITE.maxParGenrePrincipal;
