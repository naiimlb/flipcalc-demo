...
export interface Plateforme {
  id: string;
  nom: string;
  /** Identifiant du provider TMDB en région FR. */
  idTmdb: number;
  /** `true` pour les offres accessibles sans abonnement payant. */
  gratuite: boolean;
  /** Couleur de marque, utilisée pour la pastille et le dégradé. */
  couleur: string;
  /** Initiales affichées dans la pastille (aucun logo de marque copié). */
  initiales: string;
  /** Page d'accueil, pour le bouton « Ouvrir sur… ». */
  lien: string;
}
...
export const PLATEFORME_PAR_ID: Record<string, Plateforme> = Object.fromEntries(
  PLATEFORMES.map((p) => [p.id, p]),
);

/** Identifiants des offres accessibles sans abonnement. */
export const PLATEFORMES_GRATUITES: string[] = PLATEFORMES.filter((p) => p.gratuite).map((p) => p.id);

/** Correspondance provider TMDB → identifiant CinéMood. */
export const PLATEFORME_PAR_ID_TMDB: Record<number, string> = Object.fromEntries(
 ...
 */
export function plateformesEffectives(plateformesUtilisateur: string[]): string[] {
