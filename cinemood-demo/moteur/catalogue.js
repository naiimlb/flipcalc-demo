/* Généré par scripts/construire-demo.mjs — NE PAS MODIFIER À LA MAIN.
   Source : cinemood/src/data/catalogue-demo.ts
...
const RYTHMES                         = { l: 'lent', m: 'modere', r: 'rapide' };

/** Transforme une ligne du tableau brut en `Titre` normalisé. */
function parserLigne(ligne        )        {
  const c = ligne.split('|');
  const liste = (s        )           => (s ? s.split('~').filter(Boolean) : []);
  const type              = c[1] === 's' ? 'serie' : 'film';
...
export const CATALOGUE_DEMO          = LIGNES.map(parserLigne);

/** Accès direct par identifiant (`film:550`). */
export const CATALOGUE_DEMO_PAR_ID                        = Object.fromEntries(
  CATALOGUE_DEMO.map((t) => [t.id, t]),
);

/** Retrouve un titre du catalogue de démonstration. */
export function titreDemo(id        )               {
